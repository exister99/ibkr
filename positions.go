...
type Position struct {
    ...
    // --- ADDED FIELD FOR CURRENT PRICE ---
    MktPrice float64 `json:"mktPrice"`   // Current market price per share
    // -------------------------------------
...
func main() {
    ...
    for _, p := range positions {
        // --- UPDATED fmt.Printf TO INCLUDE MktPrice ---
        fmt.Printf("Symbol: %s | Qty: %.0f | Price: $%.2f | Mkt Value: $%.2f | Unrealized P&L: $%.2f\n", 
            p.Symbol, p.Position, p.MktPrice, p.MktValue, p.UnrealizedPnL)
        // ----------------------------------------------
